...
class AirTemperature(object):
    ...
    def __init__(self, startdate, enddate, *args):
        ...
        if args and args[0] != "":
            ...
            query = "SELECT DATE_TIME, PROBE_CODE, AIRTEMP_MEAN, AIRTEMP_MEAN_FLAG from LTERLogger_new.dbo.MS04311 WHERE SITECODE LIKE " + site_code + " AND DATE_TIME >= \'" + startdate + "\' AND DATE_TIME <= \'" + enddate + "\' ORDER BY DATE_TIME ASC"
        else:
    ...
    def condense_data(self):
        ...
        for probe_code in self.od.keys():
            ...
            for each_date in self.od[probe_code].keys():
                ...
                newrow = ['MS043','01', site_code, method_code, height, "1D", probe_code, datetime.datetime.strftime(each_date,'%Y-%m-%d %H:%M:%S'), mean_valid_obs, daily_flag, max_valid_obs, max_flag[0], datetime.datetime.strftime(max_valid_time[0], '%Y-%m-%d %H:%M:%S'), min_valid_obs, min_valid_time[0], min_flag[0], "EVENT_CODE"]
...
if __name__ == "__main__":
    ...
    with open('wootwootcsv.csv','wb') as writefile:
    ...
    with open('README.md','wb') as otherwritefile:
        otherwritefile.write(""" THIS IS OUR NEW TOOL FOR UPDATING FSDBDATA
                ===========================
